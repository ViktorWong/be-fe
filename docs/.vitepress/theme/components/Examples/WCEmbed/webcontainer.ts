...
import {
    vsCodeDarkPlus,
    vsCodeDarkPlusTerminalTheme,
} from "./vscode-dark-plus";
import {
    vsCodeLightPlus,
    vsCodeLightPlusTerminalTheme,
} from "./vscode-light-plus";
...
export async function init(
    ...
    return {
        ...
        updateTheme(isDark: Ref<boolean>) {
            ...
            editor.dispatch({
                effects: [
                    editorThemeConfig.reconfigure(
                        isDark.value ? vsCodeDarkPlus : vsCodeLightPlus
                    ),
                ],
            });
        },
    ...
    try {
        ...
        terminal.write(
            wordWrap(
                [
                    `\x1b[G${red(
                        `Looks like your browser's configuration is blocking WebContainers.`
                    )}`,
                    "",
